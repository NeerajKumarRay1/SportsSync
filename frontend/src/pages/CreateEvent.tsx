...
import { isFutureDate, isValidGeoBounds } from '../utils/validators';
...
export default function CreateEvent() {
  ...
  const dateError = dateTime && !isFutureDate(dateTime) ? 'Event must be in the future.' : null;
  const geoError = form.latitude !== null && form.longitude !== null && !isValidGeoBounds(form.latitude, form.longitude) ? 'Invalid location coordinates.' : null;

  const isFormValid = dateTime && !dateError && !geoError && form.latitude !== null && form.longitude !== null;
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${dateError ? 'border-red-500' : 'border-gray-200'}`}
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-5 py-4">
        {step === 'details' ? (
          <button
            ...
            disabled={!!dateError || !form.date || !form.time}
            className={`w-full py-4 text-white font-semibold rounded-2xl text-sm flex items-center justify-center gap-2 transition-colors ${
              !dateError && form.date && form.time
          ...
          <div className="flex flex-col gap-2">
            {geoError && <p className="text-red-500 text-xs text-center">{geoError}</p>}
            <button
              onClick={handleSubmit}
              disabled={isSubmitting || !isFormValid}
              className={`w-full py-4 font-semibold rounded-2xl text-sm transition-colors ${
